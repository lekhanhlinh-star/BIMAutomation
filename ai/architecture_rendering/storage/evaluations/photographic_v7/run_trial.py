...
options=RenderOptions(view_type='exterior',geometry_mode='staged',quality='final_2k',render_purpose='client_presentation',
...
async def main():
 settings.DEFAULT_IMAGE_MODEL='gemini-3-pro-image'
 ...
 if variant == 'daylight':
  config=config.model_copy(update={'lighting':'Ban ngày nắng trong, ánh sáng trắng tự nhiên; bóng cây rõ nhưng vùng tối vẫn có ánh sáng bầu trời. Đèn hiện có sáng nhẹ.', 'custom_prompt':'Bậc cửa hiện có là đá xám đậm, lòng đường là nhựa đường. Thể hiện như ảnh chụp công trình hoàn thiện, vật liệu tự nhiên và bối cảnh có sức sống; không đổi kiến trúc.'})
