...
import { Award, CheckCircle2, ChevronRight, Download, Headphones, LogOut, Mail, MapPin, Menu, PhoneCall, ShieldCheck, Sparkles, User, X, Zap } from 'lucide-react';
...
export default function PublicLayout() {
  ...
  const NavLink = ({ path, label, mobile = false }) => {
    ...
    return (
      <Link
        ...
        className={`${
          mobile
            ? 'flex items-center px-4 py-3 rounded-lg font-semibold text-sm'
            : 'relative px-3.5 py-1.5 rounded-full text-sm font-medium transition-all'
        } ${
          isActive
            ? 'text-[var(--brand)] font-bold bg-[var(--brand-soft)] shadow-xs'
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--surface)] text-[var(--text-primary)] antialiased selection:bg-[var(--brand)] selection:text-[var(--brand-text)]">
      {/* Sleek Floating Glass Navigation Bar */}
      <header className="sticky top-0 z-50 w-full bg-[var(--surface)]/80 backdrop-blur-xl border-b border-[var(--line)] transition-colors duration-200">
        <div className="page-shell h-[68px] flex items-center justify-between">
          <div className="flex items-center gap-8">
            <Link to="/" className="inline-flex items-center group" aria-label="BIMAutomation - Trang chủ">
              <BrandLogo />
            </Link>
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-[var(--radius-control)] text-xs font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-subtle)] transition-colors cursor-pointer flex items-center gap-1.5"
            >
              <Headphones size={14} className="text-[var(--brand)]" /> Nhận tư vấn
            </button>
            ...
            {isAuthenticated ? (
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="px-3.5 py-2 text-xs font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"
                >
                  Đăng nhập
                </Link>
                <Link
                  to="/download"
                  onClick={goDownload}
                  className="primary-button !min-h-10 !py-2 !px-4 text-xs font-bold shadow-sm"
                >
                  <Download size={14} /> Dùng thử miễn phí
                </Link>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 lg:hidden">
            ...
            <button
              ...
              className="w-10 h-10 grid place-items-center rounded-[var(--radius-control)] border border-[var(--line)] bg-[var(--surface-raised)] text-[var(--text-primary)]"
            >
        ...
        {open && (
          <nav
            ...
            className="lg:hidden absolute inset-x-0 top-[68px] bg-[var(--surface-raised)] border-b border-[var(--line)] p-4 shadow-xl animate-fade-in"
          >
            <div className="grid gap-1">
              {links.map(([p, l]) => (
                <NavLink key={p} path={p} label={l} mobile />
              ))}
              <div className="border-t border-[var(--line)] mt-3 pt-3 grid gap-2">
                <button
                  onClick={() => {
                    setOpen(false);
                ...
                >
                  <Headphones size={16} /> Đăng ký nhận tư vấn
                </button>
                {isAuthenticated ? (
                  <>
      ...
      {/* Main Outlet with smooth overflow container */}
      ...
      <footer className="border-t border-[var(--line)] bg-[var(--surface-raised)] pt-16 pb-12 text-sm transition-colors">
        <div className="page-shell grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 pb-14 border-b border-[var(--line)]">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <p className="text-xs sm:text-sm text-[var(--text-secondary)] leading-relaxed max-w-md">
              Hệ sinh thái Add-in tự động hóa thiết kế, mô hình và xuất bản hồ sơ Autodesk Revit chuẩn xác dành cho kỹ sư, kiến trúc sư và doanh nghiệp tổng thầu tại Việt Nam.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-xs sm:text-sm text-[var(--text-secondary)]">
              <li><Link to="/features" className="hover:text-[var(--brand)] transition-colors">Tự động Dim kích thước</Link></li>
              <li><Link to="/features" className="hover:text-[var(--brand)] transition-colors">Đổi tên Sheet/View hàng loạt</Link></li>
              <li><Link to="/features" className="hover:text-[var(--brand)] transition-colors">Đồng bộ tham số 2 chiều Excel</Link></li>
              <li><Link to="/features" className="hover:text-[var(--brand)] transition-colors">Tự động xuất PDF / DWG / IFC</Link></li>
              <li><Link to="/features" className="hover:text-[var(--brand)] transition-colors">Khống chế cốt thép Kết cấu</Link></li>
              <li><Link to="/features" className="hover:text-[var(--brand)] transition-colors">Tự động đi tuyến ống MEP</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-xs sm:text-sm text-[var(--text-secondary)]">
              <li><Link to="/pricing" className="hover:text-[var(--brand)] transition-colors">Bảng giá bản quyền</Link></li>
        ...
        <div className="page-shell pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[var(--text-muted)]">
          <p>© 2026 BIMAutomation (BIMPilot). Bản quyền thuộc về Đội ngũ Kỹ thuật BIM Việt Nam.</p>
